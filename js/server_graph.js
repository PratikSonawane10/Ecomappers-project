$(function() {
	...
	var url = "/EcomappersAPI/api/ecomappersapi.php?method=showSensorDataPoints&format=json"; //../ecoAPI/dev
        
    jsonData();
    //Take the graph data
    function jsonData() {
		$.getJSON(url, function(response) {
	       	if($.isArray(response.showPollutionSensorDetailsResponse) && response.showPollutionSensorDetailsResponse.length) {
				//var env = environment.getEnv();
				var Sensor1 = [];
				var Time = [];
				$.each(response.showPollutionSensorDetailsResponse, function(i, sensorPoints) {
	...
	$("#sensordateandtimerange").change(function(){
        ...
        url = "/EcomappersAPI/api/ecomappersapi.php?method=showSensorDataPointsAsPerDateRange&format=json&fromDate="+$fullFromDateWithTime+"&toDate="+$fullToDateWithTime+""; //../ecoAPI/dev
        jsonData();
	});
